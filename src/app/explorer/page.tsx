    ...
    async function fetchLiveLedger() {
      try {
        ...
        if (isMounted) {
          ...
          setLiveState({
            submissionCount: (parsed.submissionCount ?? parsed.claimCount ?? 0n).toString(),
            ...
            scoreThresholdLimit: (parsed.scoreThresholdLimit ?? parsed.minimumRequiredDays ?? 10n).toString(),
          });
        }
      } catch (err: any) {
        if (isMounted) {
    ...
    <div style={{ maxWidth: 860, margin: "0 auto", padding: "2rem 1.5rem 4rem" }}>
      <div style={{ marginBottom: "2rem" }}>
        ...
        <p className="section-desc">
          Live on-chain state of the Anonymous Mental Health Survey ZK contract on Midnight Preview.
        </p>
      </div>
      ...
      <div className="glass-card" style={{ padding: "1.5rem", marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          {[
            { field: "submissionCount: Counter", val: liveState?.submissionCount, desc: "Total verified anonymous survey submissions", color: "#e11d48" },
            { field: "revokedCount: Counter", val: liveState?.revokedCount, desc: "Total revoked or voided submissions", color: "#ef4444" },
            { field: "activeSession: Counter", val: liveState?.activeSession, desc: "Epoch nonce (replay protection)", color: "#06b6d4" },
            { field: "cohortId: Bytes<32>", val: liveState?.cohortId, desc: "Active survey cohort identifier", color: "#10b981" },
            { field: "surveyorCommitment: Bytes<32>", val: liveState?.surveyorCommitment, desc: "Surveyor public authority anchor", color: "#f59e0b" },
            { field: "lastSubmissionCommitment: Bytes<32>", val: liveState?.lastSubmissionCommitment, desc: "Most recent ZK survey submission hash", color: "#8b5cf6" },
            ...
            { field: "scoreThresholdLimit: Uint<32>", val: liveState ? liveState.scoreThresholdLimit + " pts" : undefined, desc: "Minimum assessment validity score required", color: "#06b6d4" },
          ].map((f) => (
            <div
              key={f.field}
              style={{
